...
def mock_codex() -> MagicMock:
    codex = MagicMock()
    # default behavior
    codex.search = AsyncMock(return_value=[])
    return codex
...
@pytest.mark.asyncio
async def test_resolve_empty_text(mock_sentence_transformer: MagicMock, mock_codex: MagicMock) -> None:
...
@pytest.mark.asyncio
async def test_resolve_no_candidates(mock_sentence_transformer: MagicMock, mock_codex: MagicMock) -> None:
    linker = VectorLinker(codex_client=mock_codex)
...
@pytest.mark.asyncio
async def test_resolve_success(mock_sentence_transformer: MagicMock, mock_codex: MagicMock) -> None:
    linker = VectorLinker(codex_client=mock_codex)
...
@pytest.mark.asyncio
async def test_resolve_malformed_candidates(mock_sentence_transformer: MagicMock, mock_codex: MagicMock) -> None:
...
@pytest.mark.asyncio
async def test_resolve_tie_handling(mock_sentence_transformer: MagicMock, mock_codex: MagicMock) -> None:
...
@pytest.mark.asyncio
async def test_resolve_reordering_logic(mock_sentence_transformer: MagicMock, mock_codex: MagicMock) -> None:
...
# Caching tests removed temporarily as caching was disabled for async refactor
# TODO: Restore tests when async caching is implemented


@pytest.mark.asyncio
async def test_rerank_empty_candidates(mock_sentence_transformer: MagicMock, mock_codex: MagicMock) -> None:
